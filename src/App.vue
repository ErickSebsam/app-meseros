  ...
  <div class="todo">
    ...
    <header class="header">
      <div class="header-top">
        ...
        <div class="search-wrapper">
          ...
          <input class="search-input" v-model="searchQuery" @input="applyFilters" placeholder="Buscar comida..." />
      ...
      <div class="categories">
        <button class="pill" :class="activeCategory === '' ? 'pill-active' : ''"
          @click="activeCategory = ''; applyFilters()">Todas</button>
        <button v-for="cat in categories" :key="cat" class="pill" :class="activeCategory === cat ? 'pill-active' : ''"
          @click="activeCategory = cat; applyFilters()">{{ cat }}</button>
      </div>
    </header>
    ...
    <aside class="sidebar" :class="cartOpen ? 'sidebar-open' : ''">
      ...
      <div class="sidebar-items" v-if="getCartTotal() > 0">
        <div v-for="product in products" :key="product.id" class="sidebar-item" v-show="cart[product.id] > 0">
    ...
    <main class="comida">
      ...
      <div class="productos">
        <div class="card" v-for="product in filteredProducts" :key="product.id">
          ...
          <button class="btn-cart" @click="sendToCart(product)"
            :disabled="!selected[product.id] || selected[product.id] === 0 || product.stock === 0"
            :class="selected[product.id] > 0 ? 'btn-cart-active' : ''">🛒</button>
          ...
          <div class="card-body">
            ...
            <div class="price-row">
              ...
              <div class="quantity-controls" v-if="product.stock > 0">
                <button class="qty-btn" @click="decreaseSelected(product)"
                  :disabled="!selected[product.id] || selected[product.id] === 0">−</button>
                <span class="qty-count">{{ selected[product.id] || 0 }}</span>
                <button class="qty-btn" @click="increaseSelected(product)"
                  :disabled="(selected[product.id] || 0) >= product.stock">+</button>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="field">
            ...
            <div class="img-preview" v-if="newProduct.image">
              <img :src="newProduct.image" alt="Preview" @error="newProduct.imageError = true"
                @load="newProduct.imageError = false" />
...
const defaultProducts = [
  ...
  { id: 14, category: 'Sushi', name: 'Salmón Nigiri (x4)', description: 'Rectángulos de arroz de sushi coronados con salmón fresco y wasabi.', price: 8.00, stock: 15, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQSGjeRoObjdPjDPMLH8GyTsqa65VLAHg8YvA&s' },
