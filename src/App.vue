<template>
  <div class="todo">
      ...
      <div class="productos">
        <div class="card" v-for="product in filteredProducts" :key="product.id">
          ...
          <button
            ...
            :class="product.stock > 0 ? 'btn-cart-active' : 'btn-cart-empty'"
          >🛒</button>
          ...
          <div class="card-body">
            ...
            <div class="price-row">
              ...
              <span v-if="product.stock === 0" class="sin-stock-label">Agotado</span>
              <span v-else class="qty-in-cart" v-show="cart[product.id] > 0">
    ...
    <!-- ── LOADER FACTURA ────────────────────────────────────── -->
    <div class="modal-overlay" v-if="facturaLoading">
      <div class="factura-loader">
        <div class="spinner"></div>
        <p>Generando tu factura...</p>
      </div>
    </div>
...
function resetStock() {
  ...
  products.value = products.value.map(p => {
    if (!defaultIds.has(p.id)) return p
...
@media (max-width: 600px) {
  .productos { grid-template-columns: repeat(2, 1fr); gap: 0.75rem; }
  .header { padding: 0.75rem 1rem 0; }
  .header-top { flex-wrap: wrap; gap: 0.6rem; }
  .logo { order: 1; flex: 1; }
  .btn-reset-stock { order: 2; }
  .cart-btn { order: 3; width: 42px; height: 42px; font-size: 1.1rem; }
  .search-wrapper { order: 4; width: 100%; flex: none; height: 42px; margin-bottom: 0.2rem; }
  .logo-img { height: 44px; }
  .comida { padding: 1rem; }
  .comida h1 { font-size: 1.2rem; }
  .sidebar { width: 100vw; }
  .pill { padding: 8px 12px; font-size: 0.8rem; }
  .product-name { font-size: 0.95rem; }
  .product-description { font-size: 0.82rem; }
  .price { font-size: 0.95rem; padding: 5px 14px; }
  .image-wrapper { width: 100px; height: 100px; }
}

@media (max-width: 420px) {
  .productos { grid-template-columns: 1fr; gap: 0.75rem; }
  .product-name { font-size: 1.05rem; }
  .product-description { font-size: 0.88rem; }
  .logo-img { height: 40px; }
  .search-wrapper { height: 40px; padding: 0 0.75rem; }
  .search-input { font-size: 0.85rem; }
  .cart-btn { width: 40px; height: 40px; }
  .comida h1 { font-size: 1.1rem; margin-bottom: 1rem; }
  .pill { padding: 9px 12px; font-size: 0.8rem; }
  .image-wrapper { width: 110px; height: 110px; }
}

@media (max-width: 320px) {
  .productos { grid-template-columns: 1fr; gap: 0.5rem; }
  .header { padding: 0.5rem 0.75rem 0; }
  .logo-img { height: 34px; }
  .cart-btn { width: 34px; height: 34px; font-size: 0.95rem; }
  .search-wrapper { height: 36px; }
  .comida { padding: 0.75rem; }
  .price { font-size: 0.88rem; padding: 4px 12px; }
  .sidebar { width: 100vw; }
}
